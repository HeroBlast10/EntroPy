...
from entropy.utils.io import load_config, load_parquet, resolve_data_path
...
def generate_report(
    ...
    if daily_pnl is not None:
        from entropy.evaluation.plots import plot_nav, plot_drawdown
    ...
    # ============================
    if daily_pnl is not None:
        from entropy.evaluation.analytics import monthly_return_table
        from entropy.evaluation.plots import plot_monthly_heatmap
    ...
    # ============================
    if daily_pnl is not None:
        from entropy.evaluation.plots import plot_rolling_sharpe
    ...
    if daily_weights is not None:
        from entropy.evaluation.analytics import rolling_turnover
        sections.append("<h2>5. Turnover Analysis</h2>")
        to = rolling_turnover(daily_weights)
        if len(to) > 0:
            from entropy.evaluation.plots import _apply_style
            _apply_style()
    ...
    if factors is not None and prices is not None and signal_col:
        ...
        try:
            ...
            from entropy.evaluation.plots import plot_ic_series, plot_quantile_returns
    ...
    if factors is not None:
        ...
        try:
            from entropy.evaluation.analytics import factor_correlation
            from entropy.evaluation.plots import plot_correlation_heatmap
    ...
    if trades is not None and not trades.empty:
        ...
        try:
            from entropy.trading.pnl import cost_attribution
    ...
    if run_walkforward and factors is not None and prices is not None and signal_col:
        ...
        try:
            from entropy.evaluation.walkforward import WalkForwardConfig, run_walk_forward
            from entropy.evaluation.plots import plot_walkforward_sharpe
    ...
    if run_ablation and daily_weights is not None and trades is not None and prices is not None:
        ...
        try:
            from entropy.evaluation.ablation import run_cost_ablation
            from entropy.evaluation.plots import plot_ablation
